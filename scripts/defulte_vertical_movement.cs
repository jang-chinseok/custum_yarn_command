...
public class defulte_vertical_movement : MonoBehaviour
{
    ...
    public float moveSpeed = 3; // 상하 이동 속도
    Vector3 startLocation; // 오브작트의 이동 시작 위치
    ...
    {   
        ...
        if (moveDiraction == true){ // 정방향이동
            this.transform.position = Vector3.MoveTowards (vector3Now, moveDir, moveSpeed*Time.deltaTime);
            Debug.Log($"{moveDir[1]- vector3Now[1]}현재 값");
            if((moveDir[1]-vector3Now[1])<0.5){ //거의 다 도착하면 역방향이동으로 전환
                moveDiraction = false;    //거의 다 도착하면 역방향이동으로 전환
            }
        }
        else { // 역방향 이동.

            this.transform.position = Vector3.MoveTowards (vector3Now, startLocation, moveSpeed*Time.deltaTime);
            if((vector3Now[1]-startLocation[1]) < 0.5){  //거의 다 도착하면 정방향 이동으로 전환
